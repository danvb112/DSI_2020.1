...
class _WordPairListPageState extends State<WordPairListPage> {
  ...
  _saveOnFirestore(int index, DSIWordPair wordPair) {
    Firestore db = Firestore.instance;

    db
        .collection('WordPairs')
        .document("Pair${index}")
        .setData({"Word${index}": "${wordPair}"});
    setState(() {});
  }

  _deleteOnFirestore(int index) {
    Firestore db = Firestore.instance;

    db.collection('WordPairs').document("Pair${index}").delete();
    setState(() {});
  }
  ...
  Widget _buildRow(BuildContext context, int index, DSIWordPair wordPair) {
    return ListTile(
      ...
      trailing: TextButton(
          onPressed: () => _saveOnFirestore(index, wordPair),
          child: Icon(Icons.save)),
      onLongPress: () => {_deleteOnFirestore(index)},
