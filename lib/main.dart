...
import 'package:dsi_2020_1/editPage.dart';
...
class _RandomWordsListPageState extends State<RandomWordsListPage> {
  ...
  Iterable<WordPair> get items {
    if (widget._filter == null) {
      ...
      //a linah abaixo retorna os pares filtras opelo filtro
  ...
  Widget _buildRow(int index, WordPair wordPair) {
    return ListTile(
        ...
        onLongPress: () =>
            {print(wordPair), wordPairs.remove(wordPair), setState(() {})},
        onTap: () async {
          final newPair = await Navigator.push(context,
              MaterialPageRoute(builder: (context) => EditPage(wordPair)));
          if (newPair == null) {
            return null;
          }
          print(newPair);
          setState(() {
            wordPairs[wordPair] = wordPairs[newPair];
          });
        });
  }
}
